...
function drillPanel(){
  ...
  const cards=qs.map((q,i)=>{
    ...
    return `<fieldset class="daily-question"><legend><span class="daily-number">${i+1}</span> <span class="daily-stem">${esc(q.stem)}</span></legend><p class="dash-note">${esc(q.topic)}</p>${figure(q.assets)}<div class="daily-options">${q.type==='mcq'?options:`<label class="field">Your answer<input class="gridin" data-drill-text="${esc(q.id)}" value="${esc(selected||'')}" ${d.completed?'disabled':''}></label>`}</div>${feedback}</fieldset>`;
  }).join('');
...
function dailyPanel() {
  ...
  const questions = (d.quiz || []).map((q,i) => {
    ...
    return `<fieldset class="daily-question"><legend><span class="daily-number">${i+1}</span> <span class="daily-stem">${esc(q.stem)}</span></legend><p class="dash-note">${esc(q.topic||'Practice')}</p>${figure(q.assets)}<div class="daily-options">${options}</div>${result}</fieldset>`;
  }).join('');
