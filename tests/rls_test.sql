...
begin
  ...
  delete from public.exams      where title like 'TEST %';
  delete from public.questions  where topic = 'TESTFIX';
  ...
  insert into public.groups(track_id, name) values ('sat','TEST SAT group') returning id into g_sat;
  insert into public.groups(track_id, name) values ('est','TEST EST group') returning id into g_est;
  ...
  insert into public.questions(track_id, topic, type, stem)
    values ('sat','TESTFIX','grid_in','Write three quarters as a decimal.') returning id into qb;
  insert into public.question_keys values (qb, '["3/4","0.75",".75"]', 'Three quarters is 0.75.');

  insert into public.questions(track_id, topic, type, stem, choices)
    values ('sat','TESTFIX','mcq','Unassigned question','[{"key":"A","text":"x"}]') returning id into qd;
  insert into public.question_keys values (qd, '"A"', 'Belongs to no exam.');

  -- EST question
  insert into public.questions(track_id, topic, type, stem, choices)
    values ('est','TESTFIX','mcq','10% of 50 = ?',
...
do $$ declare sat_full int; sat_half int; est_full int; est_half int; begin
  ...
  perform public.t_ok('12 SAT scale is 200-800 in steps of 10',
                      sat_full = 800 and sat_half = 500,
                      format('full=%s half=%s', sat_full, sat_half));
  perform public.t_ok('12b EST scale is independent of SAT',
                      est_full = 400 and est_half = 250,
...
do $$ declare f int; t int; begin
  ...
  if f > 0 then
    raise exception '% of % ACCEPTANCE TESTS FAILED', f, t;
  end if;
...
drop function if exists public.t_ok(text, boolean, text);
